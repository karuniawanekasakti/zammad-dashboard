...
from app.models import ApiResponse, KpiSummary, TicketOut, TrendPoint
...
async def kpi_summary(current: Annotated[dict, Depends(get_current_user)]):
    ...
    kpi = KpiSummary(
        ...
        agents_online=len([u for u in users if u.get("active", True)]),
...
async def sla_breach_rate(current: Annotated[dict, Depends(get_current_user)], days: int = Query(30, ge=1, le=90)):
    tickets = [t for t in await _fetch_scoped(current) if t.first_response_breached or t.close_breached or t.sla_status == "breached"]
...
async def resolution_time(current: Annotated[dict, Depends(get_current_user)], days: int = Query(14, ge=1, le=90)):
    tickets = await _fetch_scoped(current)
...
async def first_reply(current: Annotated[dict, Depends(get_current_user)], days: int = Query(14, ge=1, le=90)):
    tickets = await _fetch_scoped(current)
