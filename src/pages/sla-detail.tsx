...
export default function SlaDetailPage({ isInline = false, ticketId }: SlaDetailPageProps) {
  ...
  // Only the ticket itself gates the shell: history, monitor, policies and
  // config are independent regions and must not hold the page back or make an
  // unrelated failure look like a missing ticket.
  ...
  const now = new Date();
  // A monitor failure is not the same as an out-of-date dataset: only a
  // settled, fresh response can make a verdict available, and the error is
  // surfaced separately below rather than silently reading as "unavailable".
  const monitorSettled = !monitor.isLoading && !monitor.isError;
  const verdictsAvailable = monitorSettled && slaVerdictsAvailable(monitor.data?.freshness);
  ...
  const content = <div className="space-y-6">
    ...
    {!verdictsAvailable && monitor.isPending && !history.isPending && <div className="text-sm text-muted-foreground" role="status">Checking SLA dataset freshness…</div>}
    {!verdictsAvailable && history.isPending && <div className="text-sm text-muted-foreground">Loading ticket history…</div>}
    {monitor.isError && <Card className="border-destructive/50 bg-destructive/5"><CardContent className="flex flex-wrap items-center gap-x-3 gap-y-2 py-4 text-sm" role="alert"><AlertTriangle className="size-4 shrink-0 text-destructive" /><div><p className="font-medium text-destructive">SLA verdict unavailable</p><p className="text-muted-foreground">The SLA monitor request failed, so no verdict can be computed. This is not a claim that the ticket is compliant.</p></div><Button variant="outline" size="sm" onClick={() => monitor.refetch()}>Retry</Button></CardContent></Card>}
    {!verdictsAvailable && !monitor.isError && !monitor.isPending && <Card className="border-amber-500/50 bg-amber-500/5"><CardContent className="flex gap-3 py-4 text-sm"><AlertTriangle className="mt-0.5 size-4 shrink-0 text-amber-600" /><div><p className="font-medium">SLA verdict unavailable</p><p className="text-muted-foreground">The synchronized dataset is not up to date, so this page will not present a stale verdict as current.</p></div></CardContent></Card>}
    ...
    {!isInline && <div className="flex flex-col gap-4 border-b pb-6 sm:flex-row sm:items-start sm:justify-between">
      ...
      {zammadBase ? <Button asChild variant="outline"><a href={`${zammadBase}/#ticket/zoom/${row.zammad_id}`} target="_blank" rel="noreferrer">Open in Zammad <ExternalLink /></a></Button> : config.isLoading ? <span className="text-xs text-muted-foreground">Loading Zammad link…</span> : config.isError ? <span className="text-xs text-destructive" role="alert" title="GET /system/config failed">Zammad link unavailable — configuration could not be loaded.</span> : null}
    </div>}
