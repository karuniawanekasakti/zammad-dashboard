...
from app.routers.tickets import OPEN_STATES, _effective_sla_status, _fetch_scoped
...
async def kpi_summary(current: Annotated[dict, Depends(get_current_user)], db: Annotated[AsyncSession, Depends(get_db)]):
    ...
    breached = [t for t in tickets if _effective_sla_status(t, now) == "breached"]
    ...
    kpi = KpiSummary(
        ...
        at_risk=len([t for t in tickets if _effective_sla_status(t, now) in ("warning", "critical")]),
    )
...
async def sla_breach_rate(current: Annotated[dict, Depends(get_current_user)], db: Annotated[AsyncSession, Depends(get_db)], days: int = Query(30, ge=1, le=90)):
    now = datetime.now(timezone.utc)
    tickets = [t for t in await _fetch_scoped(current, db) if _effective_sla_status(t, now) == "breached"]
